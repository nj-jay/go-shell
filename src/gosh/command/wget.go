...
//打印时间
	...
	} else if len(argv) >= 4 && argv[1] == "-d" {

		for _, url := range argv[3:] {
...
func getMultiple(url , path string) string{
	...
	if res.StatusCode == 404 {
		...
		//当前目录创建文件
		downFile, _ := os.Create(path + "/" + url[index+1:])
